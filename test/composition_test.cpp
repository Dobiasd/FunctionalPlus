...
TEST_CASE("composition_test - forward_apply")
{
    ...
    REQUIRE_EQ(forward_apply(3, [](auto x)
                             { return x * x; }),
               9);
}
...
TEST_CASE("composition_test - parameter_binding")
{
    ...
    REQUIRE_EQ(bind_2nd_of_2([](auto x, auto y)
                             { return x / y; },
                             2)(6),
               3);
    ...
    { return x + y + z; };
    auto genericAdd3 = [](auto x, auto y, auto z)
    { return x + y + z; };
...
TEST_CASE("composition_test - flip")
{
    ...
    auto APlusTwoTimesB = [](int a, int b)
    { return a + 2 * b; };
    auto TwoTimesAPlusB = [](int a, int b)
    { return 2 * a + b; };
    auto Minus = [](auto a, auto b, auto c)
    { return a - b - c; };
...
TEST_CASE("composition_test - apply_to_pair")
{
    ...
    auto APlusTwoTimesB = [](int a, int b)
    { return a + 2 * b; };
    auto APlusTwoTimesBGenericLambda = [](auto a, auto b)
    { return a + 2 * b; };
...
TEST_CASE("composition_test - memoize")
{
    ...
    auto g = memoize([&g_call_cnt](int x)
                     { ++g_call_cnt; return x * x; });
    REQUIRE_EQ(g(2), 4);
    REQUIRE_EQ(g(2), 4);
    REQUIRE_EQ(g(3), 9);
    REQUIRE_EQ(g(3), 9);
