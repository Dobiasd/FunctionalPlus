...
public:
    explicit CompositionTestState(int x)
        : x_(x)
    {
    }
    void Add(int y) { x_ += y; }
...
TEST_CASE("composition_test - parameter_binding")
{
    ...
    auto squareRowElems = bind_1st_of_2(transform<decltype(square), const Row&>, square);
...
TEST_CASE("composition_test - constructor_as_function")
{
    ...
    struct two_ctors {
        ...
        two_ctors(std::initializer_list<int>)
            : val_(2)
        {
        }
        int val_;
    };
    const two_ctors one_or_two = constructor_as_function<two_ctors, int, int>(3, 4);
