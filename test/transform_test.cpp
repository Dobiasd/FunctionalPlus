...
TEST_CASE("transform_test - transform")
{
    ...
    auto intTimes2 = [](int x) -> int
    { return x * 2; };
    auto intTimes3 = [](int x) -> int
    { return x * 3; };

    REQUIRE_EQ(transform([](auto x)
                         { return x * x; },
                         xs),
               IntVector({1, 4, 4, 9, 4}));
...
TEST_CASE("transform_test - custom_allocator")
{
    ...
    special_vector<std::string> ys = fplus::transform([](auto x)
