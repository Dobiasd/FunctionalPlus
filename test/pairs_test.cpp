...
TEST_CASE("pairs_test - zip_with")
{
    ...
    const auto multiply = [](int x, int y)
    { return x * y; };
    ...
    const auto add_generic = [](auto x, int y)
    { return x + y; };
...
{
    using namespace fplus;
    const auto multiply = [](int x, int y, int z)
    { return x * y * z; };
    const auto multiply_generic = [](auto x, auto y, auto z)
    { return x * y * z; };
...
TEST_CASE("pairs_test - zip_with_defaults")
{
    ...
    const auto add = [](int x, int y)
    { return x + y; };
    const auto add_generic = [](auto x, auto y)
    { return x + y; };
...
TEST_CASE("pairs_test - pair functions")
{
    ...
    REQUIRE_EQ(transform_fst([](auto i)
                             { return i * i; },
                             intPair),
               std::make_pair(4, 3));
    REQUIRE_EQ(transform_snd([](auto i)
                             { return i * i; },
                             intPair),
               std::make_pair(2, 9));
