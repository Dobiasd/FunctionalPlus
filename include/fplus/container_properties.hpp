...
// API search type: minimum_idx_by_unsafe : (((a, a) -> Bool), [a]) -> Int
// fwd bind count: 1
// Return the index of the first minimum element using a less comparator.
// minimum_idx_by_unsafe(lessLength, ["123", "12", "1234", "123"]) -> "1"
template <typename Compare, typename Container>
typename std::size_t minimum_idx_by_unsafe(Compare comp,
        const Container& xs)
{
...
// minimum_idx_by(lessLength, ["123", "12", "1234", "123"]) -> Just "1"
// minimum_idx_by(lessLength, []) -> Nothing
template <typename Compare, typename Container>
maybe<typename std::size_t> minimum_idx_by(Compare comp,
        const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return minimum_idx_by_unsafe(comp, xs);
}

// API search type: maximum_idx_by_unsafe : (((a, a) -> Bool), [a]) -> Int
// fwd bind count: 1
// Return the index of the first maximum element using a less comparator.
// maximum_idx_by_unsafe(lessLength, ["123", "12", "1234", "123"]) == "2"
template <typename Compare, typename Container>
typename std::size_t maximum_idx_by_unsafe(Compare comp,
        const Container& xs)
{
    internal::check_compare_for_container<Compare, Container>();
    assert(is_not_empty(xs));
...
// API search type: maximum_idx_by : (((a, a) -> Bool), [a]) -> Maybe Int
// fwd bind count: 1
// Return the index of the first maximum element using a less comparator
// if sequence is not empty.
// maximum_idx_by(lessLength, ["123", "12", "1234", "123"]) == Just "2"
// maximum_idx_by(lessLength, []) == Nothing
template <typename Compare, typename Container>
maybe<typename std::size_t> maximum_idx_by(Compare comp,
        const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return maximum_idx_by_unsafe(comp, xs);
}


// API search type: minimum_idx_unsafe : [a] -> Int
// fwd bind count: 0
// Return the index of the first minimum element.
// minimum_idx_unsafe([3, 1, 4, 2]) == 1
template <typename Container>
typename std::size_t minimum_idx_unsafe(const Container& xs)
{
    return minimum_idx_by_unsafe(is_less<typename Container::value_type>, xs);
}

// API search type: minimum_idx : [a] -> Maybe Int
// fwd bind count: 0
// Return the index of the first minimum element if sequence is not empty.
// minimum_idx([3, 1, 4, 2]) == Just 1
// minimum_idx([]) == Nothing
template <typename Container>
maybe<typename std::size_t> minimum_idx(const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return minimum_idx_unsafe(xs);
}

// API search type: maximum_idx_unsafe : [a] -> Int
// fwd bind count: 0
// Return the index of the first maximum element.
// maximum_idx_unsafe([3, 1, 4, 2]) == 2
template <typename Container>
typename std::size_t maximum_idx_unsafe(const Container& xs)
{
    return maximum_idx_by_unsafe(is_less<typename Container::value_type>, xs);
}

// API search type: maximum_idx : [a] -> Maybe Int
// fwd bind count: 0
// Return the index of the first maximum element if sequence is not empty.
// maximum_idx([3, 1, 4, 2]) == Just 2
// maximum_imaximum_idxdx([]) == Nothing
template <typename Container>
maybe<typename std::size_t> maximum_idx(const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return maximum_idx_unsafe(xs);
}


// API search type: minimum_idx_on_unsafe : ((a -> b), [a]) -> Int
// fwd bind count: 1
// Return the index of the first minimum element using a transformer.
// minimum_idx_on_unsafe(length, ["123", "12", "1234", "123"]) -> "1"
template <typename F, typename Container>
typename std::size_t minimum_idx_on_unsafe(F f, const Container& xs)
{
    ...
    return minimum_idx_unsafe(transformed);
}

// API search type: minimum_idx_on : ((a -> b), [a]) -> Just Int
// fwd bind count: 1
// Return the index of the first minimum element using a transformer
// if sequence is not empty.
// minimum_idx_on(length, ["123", "12", "1234", "123"]) -> Just "1"
// minimum_idx_on(length, []) -> Nothing"
template <typename F, typename Container>
maybe<typename std::size_t> minimum_idx_on(F f, const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return minimum_idx_on_unsafe(f, xs);
}

// API search type: maximum_idx_on_unsafe : ((a -> b), [a]) -> Int
// fwd bind count: 1
// Return the index of the first maximum element using a transformer.
// maximum_idx_on_unsafe(length, ["123", "12", "1234", "123"]) == "2"
template <typename F, typename Container>
typename std::size_t maximum_idx_on_unsafe(F f, const Container& xs)
{
    typedef typename std::result_of<F(typename Container::value_type)>::type Result;
    auto transformed = transform_convert<std::vector<Result>>(f, xs);
    return maximum_idx_unsafe(transformed);
}

// API search type: maximum_idx_on : ((a -> b), [a]) -> Maybe Int
// fwd bind count: 1
// Return the index of the first maximum element using a transformer
// if sequence is not empty.
// maximum_idx_on(length, ["123", "12", "1234", "123"]) == Just "2"
// maximum_idx_on(length, []) == Nothing
template <typename F, typename Container>
maybe<typename std::size_t> maximum_idx_on(F f, const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return maximum_idx_on_unsafe(f, xs);
}

// API search type: minimum_by_unsafe : (((a, a) -> Bool), [a]) -> a
// fwd bind count: 1
// Return the first minimum element using a less comparator.
// minimum_by_unsafe(lessLength, ["123", "12", "1234", "123"]) -> "12"
template <typename Compare, typename Container>
typename Container::value_type minimum_by_unsafe(Compare comp,
        const Container& xs)
{
    internal::check_compare_for_container<Compare, Container>();
    assert(is_not_empty(xs));
...
// Return the first minimum element using a less comparator
// if sequence is not empty.
// minimum_by(lessLength, ["123", "12", "1234", "123"]) -> Just "12"
// minimum_by(lessLength, []) -> Nothing
template <typename Compare, typename Container>
maybe<typename Container::value_type> minimum_by(Compare comp,
        const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return minimum_by_unsafe(comp, xs);
}

// API search type: maximum_by_unsafe : (((a, a) -> Bool), [a]) -> a
// fwd bind count: 1
// Return the first maximum element using a less comparator.
// maximum_by_unsafe(lessLength, ["123", "12", "1234", "123"]) == "1234"
template <typename Compare, typename Container>
typename Container::value_type maximum_by_unsafe(Compare comp,
        const Container& xs)
{
    internal::check_compare_for_container<Compare, Container>();
    assert(is_not_empty(xs));
...
// API search type: maximum_by : (((a, a) -> Bool), [a]) -> Maybe a
// fwd bind count: 1
// Return the first maximum element using a less comparator
// if sequence is not empty.
// maximum_by(lessLength, ["123", "12", "1234", "123"]) == Just "1234"
// maximum_by(lessLength, []) == Nothing
template <typename Compare, typename Container>
maybe<typename Container::value_type> maximum_by(Compare comp,
        const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return maximum_by_unsafe(comp, xs);
}


// API search type: minimum_unsafe : [a] -> a
// fwd bind count: 0
// Return the first minimum element.
// minimum_unsafe([3, 1, 4, 2]) == 1
template <typename Container>
typename Container::value_type minimum_unsafe(const Container& xs)
{
    return minimum_by_unsafe(is_less<typename Container::value_type>, xs);
}

// API search type: minimum : [a] -> Maybe a
// fwd bind count: 0
...
// minimum([]) == Nothing
template <typename Container>
maybe<typename Container::value_type> minimum(const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return minimum_unsafe(xs);
}

// API search type: maximum_unsafe : [a] -> a
// fwd bind count: 0
// Return the first maximum element.
// maximum_unsafe([3, 1, 4, 2]) == 4
template <typename Container>
typename Container::value_type maximum_unsafe(const Container& xs)
{
    return maximum_by_unsafe(is_less<typename Container::value_type>, xs);
}

// API search type: maximum : [a] -> Maybe a
// fwd bind count: 0
...
// maximum([]) == Nothing
template <typename Container>
maybe<typename Container::value_type> maximum(const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return maximum_unsafe(xs);
}


// API search type: minimum_on_unsafe : ((a -> b), [a]) -> a
// fwd bind count: 1
// Return the first minimum element using a transformer.
// minimum_on_unsafe(length, ["123", "12", "1234", "123"]) -> "12"
template <typename F, typename Container>
typename Container::value_type minimum_on_unsafe(F f, const Container& xs)
{
    internal::check_arity<1, F>();
    return elem_at_idx(minimum_idx_on_unsafe(f, xs), xs);
}

// API search type: minimum_on : ((a -> b), [a]) -> Maybe a
// fwd bind count: 1
// Return the first minimum element using a transformer
// if sequence is not empty.
// minimum_on(length, ["123", "12", "1234", "123"]) -> Just "12"
// minimum_on(length, []) -> Nothing
template <typename F, typename Container>
maybe<typename Container::value_type> minimum_on(
    F f, const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return minimum_on_unsafe(f, xs);
}

// API search type: maximum_on_unsafe : ((a -> b), [a]) -> a
// fwd bind count: 1
// Return the first maximum element using a transformer.
// maximum_on_unsafe(length, ["123", "12", "1234", "123"]) == "1234"
template <typename F, typename Container>
typename Container::value_type maximum_on_unsafe(F f, const Container& xs)
{
    internal::check_arity<1, F>();
    return elem_at_idx(maximum_idx_on_unsafe(f, xs), xs);
}

// API search type: maximum_on : ((a -> b), [a]) -> Maybe a
// fwd bind count: 1
// Return the first maximum element using a transformer
// if sequence is not empty.
// maximum_on(length, ["123", "12", "1234", "123"]) == Just "1234"
// maximum_on(length, ["123", "12", "1234", "123"]) == Nothing
template <typename F, typename Container>
maybe<typename Container::value_type> maximum_on(
    F f, const Container& xs)
{
    if (is_empty(xs))
        return {};
    else
        return maximum_on_unsafe(f, xs);
}
